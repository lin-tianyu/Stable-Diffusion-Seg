...
class ISIC18Base(Dataset):
    ...
    def __init__(self, data_root, size=256, interpolation="nearest", mode=None, num_classes=2):
        ...
        print(f"[Dataset]: ISIC18 with 2 classes, in {self.mode} mode")
    ...
    def __getitem__(self, i):
        ...
        segmentation = Image.fromarray(
            cv2.cvtColor(cv2.imread(example["file_path_"].replace("Input", "GroundTruth").replace(".jpg", "_segmentation.png")),
                         cv2.COLOR_BGR2RGB))
    ...
    def _parse_data_list(self): 
        ...
        train_imgs, val_imgs, test_imgs = all_imgs[:1815], all_imgs[1815:1815+259], all_imgs[1815+259:] # split 1
        # train_imgs, val_imgs, test_imgs = all_imgs[:2364], all_imgs[2364:2364+295], all_imgs[2364+295:] # split 2
...
class ISIC18Train(ISIC18Base):
    def __init__(self, **kwargs):
        super().__init__(data_root="data/ISIC18/ISIC2018_Task1_Training_Input", mode="train", **kwargs)


class ISIC18Validation(ISIC18Base):
    def __init__(self, **kwargs):
        super().__init__(data_root="data/ISIC18/ISIC2018_Task1_Training_Input", mode="val", **kwargs)


class ISIC18Test(ISIC18Base):
    def __init__(self, **kwargs):
        super().__init__(data_root="data/ISIC18/ISIC2018_Task1_Training_Input", mode="test", **kwargs)
