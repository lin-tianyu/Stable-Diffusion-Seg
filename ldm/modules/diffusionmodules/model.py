    ...
    def init_from_ckpt(self, path):     # only conditioning model will call this method
        ...
        for k, v in sd.items():
            if k.startswith("encoder."):
                new_keys[k.replace("encoder.", "")] = v
        missing, unexpected = self.load_state_dict(new_keys, strict=False)
        print(
            f"\033[32mRestored Conditioning-model from {path} with {len(missing)} missing and {len(unexpected)} unexpected keys\033[0m")
        if len(missing) > 0:
