...
if __name__ == "__main__":
    ...
    root_path = "/GPUFS/nsccgz_ywang_zfd/LinTianyu/AIM/UniMatch/more-scenarios/medical/dataset"
    save_path = os.path.join(root_path, "data")
    root_labelled = os.path.join(root_path, "labelled")
    # root_unlabelled = os.path.join(root_path, "unlabelled")   # NO NEED
    # root_test = os.path.join(root_path, "test")               # NO NEED
    stop_idx = 10e10
    labeled_slice, all_slice = 0, 0

    """ for labeled"""
    with open("labeledall.txt", "w+") as f:
        train_path = glob.glob(os.path.join(root_labelled, "image", "**"))
        # assert len(train_path)==9
        for idx, path in tqdm(enumerate(train_path), desc="processing unlabeled data"):
    ...
    #     val_path = glob.glob(os.path.join(root_test, "image", "**"))
    #     print(val_path)
    ...
    #             hf.create_dataset(name="label", dtype=float, data=None)
    #         hf.close()
    #
    #         if idx == stop_idx:
    #             sys.exit(0)
    ...
    #             enumerate(glob.glob(os.path.join(root_unlabelled, "*.nii.gz")), ),
    #             total=len(glob.glob(os.path.join(root_unlabelled, "*.nii.gz"))),
    ...
    #                 continue
    #
    #             save_path_h5 = os.path.join(save_path, ".".join(path.split("/")[-1].split(".")[:-2]) + f"_slice_{slice_idx}.h5")
    #             f.write(
    #                 os.path.join("data", ".".join(path.split("/")[-1].split(".")[:-2]) + f"_slice_{slice_idx}.h5")+"\n"
    #             )
    #             with h5py.File(save_path_h5, mode="w") as hf:
    #                 hf.create_dataset(name="image", dtype=float, data=img_slice)
    #                 hf.create_dataset(name="label", dtype=float, data=None)
